...
1. Pick a stable reverse-DNS ID (e.g., `org.example.BigClock`) and update `application_id` in `big_clock.js`.
...
## AppStream Metadata
1. Author `data/org.example.BigClock.metainfo.xml` describing the app (summary, description, license, release notes, screenshots).
...
## Optional Schemas
If preferences are added, create `data/org.example.BigClock.gschema.xml`, install it to `/usr/share/glib-2.0/schemas/`, and invoke `glib-compile-schemas` post-install.

## Packaging Targets
1. Classic distros: wrap the build/install steps in Deb/RPM specs.
2. Flatpak: create `org.example.BigClock.yml` referencing the Meson build. Flatpak manifests unlock GNOME Software distribution by default.
